...
Additions: This fork has been modified to dynamically discover clickhouse pods that are part of the same clickhouse cluster, all deployed on kubernetes. The modifications assume there is a single clickhouse cluster running on the kubernetes cluster, and currently does not support multiple clickhouse clusters. The modifications use the kubernetes/client-go client to access the kubernetes APIs, and performs a naive pod scan looking for pods with names including or exluding single string values. By default it looks for pods with "clickhouse" in the name, and excludes pods with "tabix" in the name as testing was performed using the 8gears clickhouse helm chart (https://artifacthub.io/packages/helm/open-8gears/clickhouse). The implementation allows CHproxy to be deployed on the same kubernetes cluster as the clickhouse pods, with the pros and cons that come with that. It can also be deployed externally to the kubernetes cluster for those who prefer using a VM to host the proxy. The modifications are enabled by default, but can be disabled by setting 'kubernetespoddiscovery' to false in config.yml. The search string values are 'kubernetespodbnameinclude' and 'kubernetespodnameexclude', also configurable in config.yml. 
